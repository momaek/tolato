...
```
role:        entry | relay | landing     （节点在加速网络中的角色）
upstream_id: string | null               （上游节点 ID，entry 为 null）
```

拓扑关系通过 `upstream_id` 隐式定义：

```
入口节点 (role=entry, upstream_id=null)
  └─→ 中转A (role=relay, upstream_id=entry-1)
        ├─→ 落地1 (role=landing, upstream_id=relay-a)
        ├─→ 落地2 (role=landing, upstream_id=relay-a)
        └─→ 落地3 (role=landing, upstream_id=relay-a)
  └─→ 中转B (role=relay, upstream_id=entry-1)
        ├─→ 落地X (role=landing, upstream_id=relay-b)
        └─→ 落地Y (role=landing, upstream_id=relay-b)
```

### 3.2 链路定义

链路由拓扑自动推导：**每个节点探测其直接下游节点**。

- 入口节点 → 探测所有 relay 节点
- 中转节点 → 探测其负责的所有 landing 节点
- 落地节点 → 不主动探测
...
在 Server 端的节点管理中，用户可以给节点设置 role 和 upstream。前端 Nodes 页面增加编辑能力即可，不需要单独的拓扑配置页面。
