...
- 左侧会话与上下文栏：session 列表、候选节点、已确认目标、节点状态摘要
...
左栏先服务于 session 切换，再服务于当前会话节点上下文，不是全量节点管理入口。
...
- 控制台左侧必须体现 `session` 列表，但不能抢占主时间线的视觉主导
- 旧版 Pencil 画布需要按新的控制台信息架构重画或补充
