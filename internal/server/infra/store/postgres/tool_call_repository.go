...
INSERT INTO tool_calls (id, session_id, task_id, message_id, tool_name, arguments, args_preview, source, created_at)
VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
`, call.ID, call.SessionID, nullableString(call.TaskID), nullableString(call.MessageID), call.ToolName,
...
SELECT id, session_id, task_id, message_id, tool_name, arguments, args_preview, source, created_at
FROM tool_calls
WHERE session_id = $1
...
func scanToolCall(rows Rows) (domain.ToolCall, error) {
	...
	if err := rows.Scan(&id, &sessionID, &taskID, &messageID, &toolName, &argumentsRaw, &argsPreview, &source, &createdAt); err != nil {
