const enUS = {
  common: {
    localeLabel: 'Language',
    locales: {
      zhCN: '简体中文',
      enUS: 'English (US)',
    },
    nav: {
      console: 'Console',
      nodes: 'Nodes',
      history: 'History',
      settings: 'Settings',
    },
    buttons: {
      reset: 'Reset',
      retry: 'Retry',
      logout: 'Log out',
      saveSettings: 'Save settings',
      saving: 'Saving...',
      cancel: 'Cancel',
      reject: 'Reject',
      approve: 'Approve',
      backToConsole: 'Back to console',
      openConsole: 'Open console',
      goToConsole: 'Go to console',
      viewDetail: 'View detail',
      openInConsole: 'Open in console',
      openShell: 'Shell',
      send: 'Send',
      confirmTarget: 'Confirm target',
      viewFullPlan: 'View full plan',
      copySummary: 'Copy summary',
      reselect: 'Reselect',
      clearContext: 'Clear context',
    },
    labels: {
      all: 'all',
      noSync: 'not synced',
      notAvailable: 'n/a',
      none: 'none',
      selected: 'Selected',
      total: 'Total',
      filtered: 'Filtered',
      updated: 'Updated',
      latestUpdate: 'Latest update',
      lastSeen: 'Last seen',
      filterMode: 'Filter mode',
      search: 'Search',
      allNodes: 'all nodes',
      language: 'Language',
      agent: 'Agent',
    },
    statuses: {
      online: 'Online',
      busy: 'Busy',
      stale: 'Stale',
      offline: 'Offline',
      unknown: 'Unknown',
    },
    states: {
      dirty: 'dirty',
      saved: 'saved',
      loadingSettings: 'Loading settings...',
      loadingNodeDetail: 'Loading node detail from mock data...',
      justNow: 'just now',
      waitingForOutput: 'waiting for output...',
      enabled: 'Enabled',
      disabled: 'Disabled',
      secure: 'secure',
      attention: 'attention',
      thinking: 'Thinking',
    },
    connection: {
      connected: 'Control Server connected',
      connecting: 'Connecting...',
      reconnecting: 'Reconnecting...',
      offline: 'Offline',
      syncedRecently: 'Synced {value}',
    },
    metrics: {
      online: '{count} online',
      offline: '{count} offline',
      nodes: '{count} nodes',
      success: 'success {count}',
      failed: 'failed {count}',
      total: 'total {count}',
      risk: 'risk {value}',
      timeout: 'timeout: {value}s',
      broadcast: 'broadcast: {value}',
      lastSeen: 'Last seen {value}',
      version: 'version {value}',
    },
    values: {
      yes: 'yes',
      no: 'no',
    },
  },
  appShell: {
    subtitle: 'AI Ops Console',
    languageLabel: 'Language',
  },
  nodeOnboarding: {
    toast: {
      title: 'No nodes are connected yet',
      description:
        'Start a `tolato-nodeagent` first, then come back to Console and Nodes.',
    },
    actions: {
      openNodes: 'Open setup guide',
    },
    console: {
      title: 'No nodes available yet',
      description:
        'Connect at least one node first. Console needs a live node before target resolution, planning, and execution can continue.',
    },
    nodes: {
      title: 'Add your first node',
      description:
        'The control plane has not received any node heartbeat yet. Start the node agent on a target machine and connect it to `/ws/agent`.',
      hint: 'The default local-dev token is `agent-dev-token`. Replace it if your backend config uses a different token.',
    },
  },
  login: {
    eyebrow: 'Control Plane Access',
    title: 'Sign in before opening Console, Nodes, or Settings.',
    description:
      'The backend now requires a bearer session. After login, the frontend attaches the token to both HTTP APIs and `ws/ui`, and it returns to login when the session expires.',
    highlights: {
      control: {
        title: 'Unified control plane',
        description:
          'Console, History, and Settings now share a real user session instead of relying on local dev headers.',
      },
      execution: {
        title: 'Live execution stream',
        description:
          'After sign-in the app connects to `ws/ui` automatically and keeps receiving session summaries, state transitions, and execution output.',
      },
      audit: {
        title: 'Session-aware security',
        description:
          'Changing the password or revoking other sessions invalidates old tokens and brings the UI back to the login state.',
      },
    },
    form: {
      title: 'Sign in',
      description: 'Use the configured admin account for this control plane.',
      username: 'Username',
      usernamePlaceholder: 'Enter your username',
      password: 'Password',
      passwordPlaceholder: 'Enter your password',
      submit: 'Sign in',
      submitting: 'Signing in...',
    },
    validation: {
      required: 'Enter both username and password',
    },
  },
  console: {
    header: {
      agent: 'AI Agent',
      targetContext: 'Target Context',
      targetUnset: 'Target context: unset',
      directShell: 'Direct shell',
      directShellTitle: 'Direct shell is not available in MVP',
      directShellDescription:
        'The current interface only supports the AI Agent flow. Direct shell would bypass target confirmation, plan generation, and approval, so MVP keeps it informational only.',
      plannerHint:
        'Natural-language requests first decide whether to inspect nodes, confirm targets, generate a plan, or enter approval.',
      healthSummary: '{online} online · {offline} offline',
      broadcastAllOnline:
        'The current target covers all online nodes. Write operations are blocked by default; only read-only operations may continue.',
      broadcastMulti:
        'The current target includes multiple nodes. Write operations require elevated approval.',
      sessionUnset: 'No session selected',
      waitingInput: 'Waiting for task input',
    },
    sidebar: {
      sessionStack: 'Session Stack',
      sessionStackDescription:
        'The left panel keeps the session list and node context visible.',
      currentTargets: 'Current Targets',
      currentTargetsDescription:
        'Candidate nodes, confirmed targets, and health summary stay in sync here.',
      broadcastGuardrail: 'Broadcast Guardrail',
      allOnlineGuardrail:
        'The current target is all online nodes. Write operations are blocked by default; only read-only execution can continue.',
      multiGuardrail:
        'The current target spans multiple nodes. Write operations require elevated approval before continuing.',
      candidates: 'Candidates',
      confirmed: 'Confirmed',
      noCandidates: 'No candidate nodes right now.',
      noConfirmed: 'No confirmed target yet.',
      onlineNodes: 'Online nodes',
      busyNodes: 'Busy nodes',
      offlineNodes: 'Offline nodes',
    },
    composer: {
      title: 'Composer',
      description:
        'Send a task request. AI will decide whether to inspect nodes, generate a plan, or enter approval.',
      placeholder:
        'Send a task request. AI will first decide whether to inspect nodes, confirm targets, generate a plan, or enter approval.',
      submitFailed: 'Failed to submit',
      quickStarts: 'Quick starts',
      shortcutHint: 'Ctrl / Cmd + Enter to send',
      inputLabel: 'Task Input',
      orchestrationHint:
        'AI will resolve targets, assess risk, and choose the execution path first.',
      readyState: 'ready',
      blockedState: 'awaiting approval',
      routingBadge: 'AI routing',
      quickActions: {
        tokyoReload: 'Tokyo nginx reload',
        dockerStatus: 'Docker status',
        systemLoad: 'System load',
        networkCheck: 'Network check',
      },
    },
    sessions: {
      title: 'Sessions',
      description: 'Current sessions and backend summaries',
      total: '{count} total',
      newSession: 'New session',
      deleteSession: 'Delete session',
      emptyTitle: 'No console session is available',
      emptyDescription:
        'The backend has not initialized a console session yet. Refresh the page or restart the server.',
    },
    targetConfirmation: {
      originalTarget: 'Original target',
      eyebrow: 'Target confirmation',
      pending: 'Pending',
      titleSingle: 'Confirm the node to operate on',
      titleMulti: 'Confirm the target node set',
      description:
        'I found {count} possible target node(s), but I still need your confirmation before continuing.',
      pauseReason: 'Why execution paused',
      pauseReasonFallback:
        'The system resolved candidate nodes, but the match confidence is still too low to continue automatically.',
      pauseReasonFallbackCandidate:
        'The most likely candidate is {label} ({nodeId}), but the match confidence is still too low to continue automatically.',
      inheritedHint:
        'This candidate target also references the existing session context. Confirm whether you want to keep using it.',
      candidateNodeId: 'Node ID',
      candidateReason: 'Match reason',
      confirmCandidate: 'Use {label}',
      clearAndReselect: 'Clear and choose again',
      scopeSingle: 'Single node',
      scopeMulti: 'Multiple nodes',
      scopeAllOnline: 'All online nodes',
      sourceResolver: 'Model resolved',
      sourceManual: 'Manual target',
      sourceSessionContext: 'Session context',
      sourceNone: 'Unspecified',
      reasonManualConfirmation:
        'The system could not confidently match this node automatically, so it paused for manual confirmation.',
      reasonMatchedNodeId: 'Matched this candidate by node ID.',
      reasonMatchedHostname: 'Matched this candidate by hostname.',
      reasonMatchedRegion: 'Matched this candidate by region.',
      reasonMatchedTag: 'Matched this candidate by tag.',
      reasonAllOnline: 'This target resolution covers all online nodes.',
      reasonResolved:
        'This candidate was resolved by the backend from the current context.',
    },
    plan: {
      ready: 'Plan Ready',
      input: 'Input',
      step: 'Step {index}',
      args: 'Args',
      detailTitle: 'Plan Detail',
      approvalRequired: 'approval required',
      autoRunReady: 'auto-run ready',
    },
    approval: {
      required: 'Approval Required',
    },
    summary: {
      title: 'Summary',
      description: 'Aggregated execution results and AI summary.',
      nextSteps: 'Next Steps',
    },
    execution: {
      stream: 'Execution Stream',
    },
  },
  pages: {
    history: {
      badge: 'History',
      badgeSecondary: 'Task audit and timeline',
      title: 'Review plan, approval, execution, and audit in one place.',
      description:
        'This page keeps the task archive separate from the live console. Use the list to inspect the full lifecycle and jump back to console when you need to continue a session.',
    },
    nodes: {
      badge: 'Nodes',
      badgeSecondary: 'HTTP query surface',
      title: 'Browse nodes, inspect health, and open a target in console.',
      description:
        'The node catalog stays separate from the live console flow. Use it to filter by status, identify busy targets, and jump into detailed inspection.',
      statusSnapshot: 'Status snapshot',
      consoleShortcut: 'Console shortcut',
      consoleShortcutDescription:
        'Open the live console to continue target confirmation or inspect session history.',
      statBusy: 'Busy',
      statOnline: 'Online',
      statOffline: 'Offline',
      nodeInventory: 'Node inventory',
      nodeInventoryDescription:
        '{visible} / {total} nodes match the current filters.',
      mockDataReady: 'Mock data ready',
      searchPlaceholder: 'Search hostname, region, or tag',
      allRegions: 'All regions',
      allTags: 'All tags',
      busyOnly: 'Busy only',
      showAllBusyStates: 'Show all busy states',
      filterAll: 'All',
      filterOnline: 'Online',
      filterBusy: 'Busy',
      filterStale: 'Stale',
      filterOffline: 'Offline',
      agentTokenTitle: 'Agent Token',
      agentTokenDescription:
        'Use this token when connecting a node agent to the control plane.',
      installCommandTitle: 'Install Command',
      installCommandDescription:
        'Run this command on a target machine to install and start the node agent. Replace <BINARY_DOWNLOAD_URL> and <VERSION> with actual values.',
      copyInstallCommand: 'Copy',
      copied: 'Copied!',
    },
    nodeDetail: {
      badge: 'Node detail',
      badgeSecondary: 'Independent page',
      title: 'Deep view for a single node.',
      description:
        'This page keeps the full node state outside console flow. It is useful for investigating health, recent tasks, and operational risk signals before jumping back to a session.',
      riskSignals: 'Risk signals',
      recentTasks: 'Recent tasks',
    },
    settings: {
      badge: 'Settings',
      badgeSecondary: 'Mock editable state',
      title: 'Tune the model, security, and user preferences.',
      description:
        'Configuration stays off the live session path. These panels are intentionally simple so they can later connect to HTTP persistence without changing the page structure.',
      emptyState: 'No settings data is loaded yet. Retry the request.',
      saveSuccess: 'Settings saved',
      testFailure: 'Model connection test failed',
      passwordRequired: 'Enter the current and new password',
      passwordSame: 'The new password must differ from the current password',
      passwordChangeSuccess: 'Password changed',
      passwordChangeFailure: 'Failed to change password',
      revokeSuccess: 'Other sessions revoked',
      revokeFailure: 'Failed to revoke other sessions',
    },
  },
  history: {
    taskList: {
      title: 'Task history',
      searchPlaceholder: 'Search task title, summary, or node',
    },
    taskDetail: {
      title: 'Task detail',
      updatedAt: 'Updated {value}',
      empty: 'Select a task to inspect the full lifecycle.',
      impact: 'Impact',
      steps: 'Steps',
      plan: 'Plan',
      approval: 'Approval',
      execution: 'Execution',
      aiSummary: 'AI summary',
      toolMeta: 'Tool meta',
      toolCalls: 'Tool calls',
      toolResults: 'Tool results',
      timelineRows: 'Timeline rows',
      audit: 'Audit',
      selectFromList:
        'Select a task from the list to inspect its plan, execution, and audit trail.',
    },
  },
  nodeOverview: {
    provider: 'Provider',
    uptime: 'Uptime',
    kernel: 'Kernel',
    resourceUsage: 'Resource usage',
    agentStatus: 'Agent status',
    agentVersion: 'Agent version',
    busy: 'Busy',
    status: 'Status',
    ip: 'IP',
    upgrade: 'Upgrade',
    upgradeDialog: {
      title: 'Upgrade Node Agent',
      description: 'Download and replace the agent binary on this node.',
      downloadUrl: 'Download URL',
      downloadUrlPlaceholder: 'https://example.com/tolato-nodeagent',
      targetVersion: 'Target Version',
      targetVersionPlaceholder: 'v1.0.0',
      confirm: 'Start Upgrade',
      cancel: 'Cancel',
      success: 'Upgrade dispatched successfully',
      error: 'Failed to dispatch upgrade',
    },
  },
  settingsPanel: {
    modelConfig: {
      title: 'Model config',
      provider: 'Provider',
      model: 'Model',
      modelHelp:
        'Load the available models from the configured endpoint and pick one directly.',
      refreshModels: 'Refresh models',
      loadingModels: 'Loading models...',
      modelsEmpty: 'No models available',
      modelsPending: 'Click refresh to load model options',
      modelsUnavailable: 'Fill in the URL and API key first',
      endpoint: 'Endpoint URL',
      endpointPlaceholder: 'https://api.openai.com/v1',
      apiKey: 'API Key',
      apiKeyPlaceholder: 'Enter a new API key',
      apiKeyRetained: 'Saved already, leave blank to keep it',
      apiKeyHelp: 'Used by the backend for model requests after saving.',
      apiKeyHelpRetained:
        'An API key is already stored. Leaving this blank will keep it unchanged.',
      apiKeyConfigured: 'api key configured',
      temperature: 'Temperature',
      approvalMode: 'Approval mode',
      openaiCompatible: 'openai compatible',
      mockReady: 'mock ready',
      testConnection: 'Test connection',
      testing: 'Testing...',
      providers: {
        openai: 'OpenAI',
        devloop: 'Dev Loop',
      },
    },
    accountSecurity: {
      title: 'Account security',
      username: 'Username',
      lastLogin: 'Last login',
      auditRetentionDays: 'Audit retention days',
      mfa: 'MFA',
      currentPassword: 'Current password',
      newPassword: 'New password',
      changePassword: 'Change password',
      changingPassword: 'Changing...',
      revokeOthers: 'Revoke other sessions',
      revoking: 'Revoking...',
    },
    preferences: {
      title: 'Preferences',
      preferredRegion: 'Preferred region',
      defaultMode: 'Default mode',
      locale: 'Language',
      compactTimeline: 'Compact timeline',
      compactTimelineDescription: 'Tighter row spacing for dense sessions.',
      streamMarkdown: 'Stream markdown',
      streamMarkdownDescription: 'Use Markstream in summary views.',
      desktopFirst: 'desktop first',
      mockDriven: 'mock driven',
    },
  },
  settingsPageNav: {
    modelConfig: 'Provider, endpoint URL, and approval behavior.',
    accountSecurity: 'Account info, password rotation, and session control.',
    preferences: 'Default region, language, and UI preferences.',
  },
  nodesTable: {
    title: 'Nodes',
    cpu: 'CPU',
    memory: 'Memory',
    disk: 'Disk',
  },
  errors: {
    unexpected: 'Unexpected error',
    requestFailed: 'Request failed',
    failedToSignIn: 'Failed to sign in',
    failedToLoadModels: 'Failed to load model list',
  },
  ws: {
    disconnected: 'ws/ui disconnected',
    notConnected: 'ws/ui is not connected',
    connectionFailed: 'ws/ui connection failed',
    closedBeforeReady: 'ws/ui closed before ready',
    unsupportedAction: 'Unsupported target action.',
    candidateRequired: 'Target candidate is required.',
    approvalContextMissing: 'Approval task context is missing.',
    rejectedFromUI: 'Rejected from Console UI',
    unknownSession: 'Unknown session: {sessionId}',
    sessionBusy: 'Session is busy',
    fallbackExecutionTitle: 'Execute on target nodes',
    fallbackPlanSummary: 'Plan generated',
    fallbackPlanImpact: 'Operational plan generated by backend.',
    fallbackAutoExecHint: 'Low-risk plan will continue without approval.',
    fallbackConsoleTask: 'Console task',
    fallbackApprovalReason: 'This plan requires explicit approval before execution.',
    fallbackApprovalImpact: 'Potentially disruptive operation.',
    fallbackSummaryComplete: 'Execution completed.',
    fallbackConfirmTarget: 'Confirm target',
    fallbackTargetBasis: 'Backend resolved a target set and is waiting for confirmation.',
    fallbackInheritedHint: 'Using confirmed target context from this session.',
    derivedRunning: 'Running on {target}.',
    derivedWaitingTarget: 'Waiting for target confirmation.',
    derivedWaitingApproval: 'Plan generated, waiting for approval.',
    derivedCompleted: 'Completed.',
    derivedFailed: 'Failed and needs attention.',
    derivedWaitingInput: 'Waiting for input.',
  },
  mockWs: {
    newSession: 'New session',
    waitingInput: 'Waiting for new task input.',
    resolvingTarget: 'Resolving target nodes.',
    reasoningNoExplicitTarget: 'No explicit target in input. Checking session context for previously confirmed targets.',
    reasoningResolveTarget: 'Listing online nodes and resolving the target expression from user input to determine which machine this round targets.',
    contentReuseConfirmed: 'I will reuse the previously confirmed {targets} and continue this analysis.',
    contentResolveAndCheck: 'Let me resolve target nodes and check the available node range.',
    inheritConfirmedTargets: 'Inheriting confirmed targets: {targets}.',
    candidatesRefreshed: 'Candidate order refreshed. Please re-confirm the target.',
    targetContextCleared: 'Target context cleared.',
    planGeneratedAwaitingApproval: 'Plan generated, awaiting approval.',
    lowRiskPlanGenerated: 'Low-risk plan generated, starting execution.',
    reasoningGeneratePlan: 'Target confirmed. Generating a structured plan and assessing risk level and approval requirements.',
    contentBroadcastWriteBlocked: 'Plan generation is complete, but this is a broadcast write operation and will not continue by default.',
    contentPlanNeedsApproval: 'I have generated a plan for {target}. This operation requires your explicit approval.',
    contentPlanAutoRun: 'I have generated a read-only plan for {target}. It will continue to execute automatically.',
    broadcastWriteMVPBlocked: 'This plan involves a broadcast write operation. It is blocked by default in MVP. Please narrow the target scope.',
    planForTargetExecution: 'I have generated an execution plan for {target}. This operation affects production and requires your approval first.',
    planForTargetReadOnly: 'I have generated a read-only plan for {target}. Low risk, continuing automatic execution.',
    approvalRejected: 'Approval rejected.',
    approvalCancelled: 'Approval cancelled.',
    approvalPassedExecuting: 'Approval granted. Task is executing.',
    nextStepWatch5xx: 'Monitor upstream 5xx for 5 minutes',
    nextStepRollback: 'To rollback, regenerate a restore plan',
    nextStepUpgradeReload: 'If latency continues to rise, escalate to a reload plan',
    reasoningSummarize: 'Summarizing node execution results into a human-readable conclusion.',
    contentWriteComplete: 'Execution complete. Post-reload health check is normal. Continue monitoring upstream and 5xx changes.',
    contentReadComplete: 'Read-only diagnostics complete. No issues requiring immediate attention. Continue monitoring.',
    assistantWriteConclusion: 'Execution complete. Post-reload health check is normal. Continue monitoring upstream and 5xx changes briefly.',
    assistantReadConclusion: 'Read-only diagnostics complete. No issues requiring immediate attention. Continue monitoring.',
    summaryWriteComplete: 'Execution complete. Nginx reload successful.',
    summaryReadComplete: 'Read-only diagnostics complete.',
    candidateBusyReason: 'Online but busy. Exercise caution during broadcast execution.',
    candidateOnlineReason: 'Currently online. Can be included in broadcast target set.',
    broadcastHitBasis: 'Input "{text}" matched broadcast scope. Showing all online nodes as candidates.',
    inheritTargetReason: 'No explicit target in input. Reusing confirmed target from previous round.',
    inheritTargetBasis: 'No new target expression provided. The system will reuse the previously confirmed target set.',
    inheritTargetHint: 'Reusing confirmed target from previous round',
    tokyoActiveReason: 'Recently active. Session summary is tracking this node.',
    tokyoBackupReason: 'Backup node in the same region.',
    resolvedBasis: 'Resolved the following candidate targets from input "{text}".',
    tokyoTarget: 'Tokyo nodes',
    defaultContextReason: 'Text mentions API / Docker related context.',
    broadcastBlockedMessage: 'Broadcast write operations are blocked by default in MVP. Narrow the target scope and retry.',
    confirmBroadcastTitle: 'Confirm broadcast targets',
    confirmTargetTitle: 'Confirm target nodes',
    planWriteBroadcastSummary: 'Running pre-checks for serial write operations on {target} to assess whether to proceed.',
    planWriteSingleSummary: 'Performing Nginx reload on {target} with pre/post health checks.',
    planReadBroadcastSummary: 'Running broadcast read-only diagnostics on {target}, aggregating per-node metrics.',
    planReadSingleSummary: 'Running read-only diagnostics on {target}, summarizing current metrics.',
    impactWriteBroadcast: 'Write operation affecting {count} nodes. Blocked by default, will not proceed to execution.',
    impactWriteSingle: 'Affects single-node entry traffic, estimated 1-2 seconds.',
    impactReadBroadcast: 'Read-only execution. Sequentially sampling {count} online nodes without modifying state.',
    impactReadSingle: 'Read-only. No node state modifications.',
    autoExecutionHint: 'Low-risk read-only plan will execute automatically without approval.',
    stepInspectNginx: 'Read active connections and workers',
    stepReloadNginx: 'nginx -s reload',
    stepVerifyService: 'Check error log and upstream',
    stepInspectNode: 'Sample CPU / memory / disk',
    stepInspectService: 'Read service health and log tail',
    approvalReasonReload: 'Plan includes live service reload. Explicit approval required.',
    approvalImpactBroadcast: 'Write operation on {count} nodes',
    approvalImpactSingle: 'Single-node write operation',
    executionWriteTitle: 'Execute nginx reload',
    executionReadTitle: 'Execute read-only diagnostics',
    summaryWriteMarkdown: '{target} completed **nginx reload**. Pre/post health checks passed. No new 5xx surge detected.',
    summaryReadMarkdown: 'Read-only check on {target} completed. No issues requiring immediate attention.',
  },
} as const

export default enUS
