    ...
    async initialize() {
      ...
      client.subscribe(event => {
        if (event.type === 'session.summary.updated') {
          const index = this.sessions.findIndex(session => session.id === event.session.id)
          if (index === -1) {
            this.sessions.unshift({
              ...event.session,
              unread: event.session.id === this.activeSessionId ? 0 : Math.max(1, event.session.unread),
            })
          } else {
            const current = this.sessions[index]
            const unread = event.session.id === this.activeSessionId
              ? 0
      ...
      this.sessions = await client.requestSessionsList()
      sortSessions(this.sessions)
