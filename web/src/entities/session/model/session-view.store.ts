...
import type { ApprovalRow, SessionSnapshot, TargetCandidate, TimelineRow } from '@/shared/types/console'
import { getWSClient } from '@/shared/ws/ws-client'
import type { WSUIEvent } from '@/shared/ws/protocol'

function applySnapshotRevision(current: SessionSnapshot | undefined, incoming: SessionSnapshot) {
...
function mapRuntimeSessionStatus(status: string | undefined): SessionSnapshot['status'] {
  switch (status) {
    case 'running':
...
function pendingActionTypeForStatus(status: string | undefined): SessionSnapshot['pendingActionType'] {
  switch (status) {
    case 'paused_wait_target_confirmation':
...
export const useConsoleSessionViewStore = defineStore('console-session-view', {
  ...
  getters: {
    activeSnapshot(state) {
      return state.activeSessionId ? state.snapshots[state.activeSessionId] ?? null : null
    },
  ...
  actions: {
    async initialize() {
      ...
      client.subscribe(event => {
        if (!this.activeSessionId || !isSessionRowEvent(event) || event.sessionId !== this.activeSessionId) {
          return
        }
        ...
        if (event.type === 'timeline.row.appended') {
          ...
          snapshot.rows.push(event.row)
        }

        if (event.type === 'execution.chunk' || event.type === 'execution.finished' || event.type === 'timeline.row.updated') {
          const snapshot = this.snapshots[event.sessionId]
          if (!snapshot || event.revision < snapshot.revision) {
            return
          }
          snapshot.revision = event.revision
          const index = snapshot.rows.findIndex(row => row.id === event.row.id)
          if (index >= 0) {
        ...
        if (event.type === 'thread.target.pending' || event.type === 'thread.target.confirmed' || event.type === 'thread.target.cleared') {
          const snapshot = this.snapshots[event.sessionId]
          if (!snapshot || event.revision < snapshot.revision) {
            return
          }
          ...
          snapshot.pendingActionType = event.type === 'thread.target.pending' ? 'target_confirmation' : undefined
        }
    ...
    async switchSession(sessionId: string) {
      ...
      const watchSessionIds = listStore.sessions.filter(session => session.id !== sessionId).map(session => session.id)
      await client.updateSubscriptions({ type: 'subscriptions.update', activeSessionId: sessionId, watchSessionIds })
      const snapshot = await client.requestSessionSnapshot(sessionId)
      this.snapshots[sessionId] = applySnapshotRevision(this.snapshots[sessionId], snapshot)
    ...
    async submitMessage(text: string) {
      ...
      await getWSClient().submitMessage({ type: 'session.message.submit', sessionId: this.activeSessionId, text })
    },
    ...
    async submitApproval(action: 'approve' | 'reject' | 'cancel', approvalRow?: ApprovalRow) {
      if (!this.activeSessionId) {
        return
      }
