export default {
  common: {
    save: 'Save Changes',
    saving: 'Saving...',
    cancel: 'Cancel',
    create: 'Create',
    add: 'Add',
    remove: 'Remove',
    done: 'Done',
    verify: 'Verify',
    loading: 'Loading...',
    search: 'Search',
    actions: 'Actions',
    name: 'Name',
    status: 'Status',
    type: 'Type',
    time: 'Time',
    duration: 'Duration',
    message: 'Message',
    yes: 'Yes',
    no: 'No',
    never: 'Never',
    allStatus: 'All Status',
    online: 'Online',
    offline: 'Offline',
    active: 'Active',
    resolved: 'Resolved',
    pending: 'pending',
    delete: 'Delete',
    page: 'Page {current} of {total}',
  },

  sidebar: {
    chat: 'Chat',
    nodes: 'Nodes',
    auditLog: 'Audit Log',
    permissions: 'Permissions',
    users: 'Users',
    settings: 'Settings',
    lightMode: 'Light Mode',
    darkMode: 'Dark Mode',
    account: 'Account',
    accountSettings: 'Account settings',
    signOut: 'Sign out',
  },

  cliAuth: {
    title: 'Authorize command-line access',
    subtitle: 'A Tolato CLI on {label} is asking to sign in as you.',
    unknownMachine: 'an unnamed machine',
    account: 'Account',
    returnsTo: 'Returns to',
    permission: 'Grant this key',
    permissions: {
      readonly: {
        label: 'Read only',
        description: 'List nodes and read their status and metrics.',
      },
      writable: {
        label: 'Read and run commands',
        description: 'Everything above, plus running shell commands on nodes you can reach.',
      },
    },
    approve: 'Authorize',
    deny: 'Cancel',
    failed: 'Could not authorize the CLI. Please try again.',
    invalidRequest:
      'This authorization request is malformed or incomplete. Start it again from the CLI with `tolato auth login`.',
    revokeHint: 'A new API key is created for this machine. You can revoke it any time under Settings, API Keys.',
  },

  login: {
    sso: {
      divider: 'or',
      signIn: 'Sign in with SSO',
      errorUnavailable: 'Single sign-on is not available. Contact your administrator.',
      errorDenied: 'The sign-in was cancelled at your identity provider.',
      errorState: 'The sign-in attempt expired or could not be verified. Please try again.',
      errorNoAccount: 'No Tolato account is linked to that identity.',
      errorDisabled: 'That account is disabled.',
      errorFailed: 'Single sign-on failed. Please try again.',
    },

    title: 'Tolato',
    subtitle: 'AI-powered VPS management',
    username: 'Username',
    password: 'Password',
    signIn: 'Sign in',
    signingIn: 'Signing in...',
    errorRequired: 'Please enter username and password',
    errorFailed: 'Login failed',
    errorNetwork: 'Network error',
    meta: 'Self-hosted · Sign in to your instance',
  },

  chat: {
    newConversation: 'New Conversation',
    conversations: 'Conversations',
    noConversations: 'No conversations yet',
    selectModel: 'Select model',
    defaultNode: 'Default node',
    allNodes: 'All Nodes',
    placeholder: {
      idle: 'Send a message...',
      streaming: 'AI is responding...',
      toolExec: 'AI is processing...',
      confirming: 'Waiting for confirmation...',
      error: 'Send a message...',
    },
    emptyState: {
      greeting: 'How can I help you manage your servers?',
      subtitle: 'Tolato runs commands on your nodes via an agent. Describe what you want done.',
      checkNginx: 'Check nginx status',
      viewDisk: 'View disk usage',
      updatePackages: 'Update packages',
    },
    thinking: 'Thinking',
    thoughtForAMoment: 'Thought for a moment',
    copy: 'Copy',
    copied: 'Copied',
    aiThinking: 'AI is thinking...',
    executing: 'Executing...',
    outputTruncated: 'Showing first {shown} of {total} lines',
    viewFullOutput: 'View all',
    loadingOutput: 'Loading...',
    loadOutputFailed: 'Load failed',
    retry: 'Retry',
    noOutput: '(no output)',
    sensitiveOperation: 'Sensitive Operation',
    approve: 'Approve',
    reject: 'Reject',
    deleteMessage: 'Delete message',
    confirmDeleteMessage: 'Delete this message? This cannot be undone.',
  },

  nodes: {
    enrolIntoGroup: 'Enrol into node group',
    noGroup: 'No group',
    enrolIntoGroupHelp: 'Machines installed with this command join the group, inheriting the grants already on it.',
    title: 'Nodes',
    searchPlaceholder: 'Search nodes...',
    addNode: 'Add Node',
    addNodeDesc: 'Enter an alias for the node, then run the install command on your server.',
    aliasPlaceholder: 'Node alias (optional)',
    creating: 'Creating...',
    generateCommand: 'Generate Install Command',
    installInstruction: 'Run this command on your server to install the agent:',
    tokenExpires: 'Token expires: {expiry}',
    tokenNeverExpires: 'Token never expires',
    confirmRemove: 'Are you sure you want to remove this node?',
    noNodes: 'No nodes found',
    ip: 'IP',
    region: 'Region',
    asn: 'ASN',
    os: 'OS',
    spec: 'Spec',
    cpu: 'CPU',
    memory: 'Memory',
    disk: 'Disk',
    lastHeartbeat: 'Last Heartbeat',
    openTerminal: 'Open Terminal',
    viewDetail: 'View Details',
    editAlias: 'Edit alias',
    aliasSaveFailed: 'Failed to save alias',
    geoipAttribution: 'Region & ASN data by',
    viewTable: 'Table view',
    viewCard: 'Card view',
    expiresIn: 'Expires in {days}d',
    expiresToday: 'Expires today',
    expiredAgo: 'Expired {days}d ago',
    lastSeen: 'Last seen {time}',
    justNow: 'just now',
    minutesAgo: '{n}m ago',
    hoursAgo: '{n}h ago',
    daysAgo: '{n}d ago',
    copyIp: 'Copy IP',
    renew: 'Renew',
    perMonth: '/mo',
    perYear: '/yr',
    perCycle: '/{cycle}',
  },

  terminal: {
    connected: 'Connected',
    connecting: 'Connecting…',
    disconnected: 'Disconnected',
    reconnect: 'Reconnect',
    reconnecting: 'Reconnecting…',
    connectionLost: 'Connection lost — your input is not being sent.',
  },

  update: {
    title: 'Update available',
    upToDate: "You're on the latest version",
    current: 'Current',
    latest: 'Latest',
    promptHint: 'Copy this and send it to the AI assistant — it can upgrade Tolato for you:',
    upgradePrompt:
      'Please upgrade Tolato to the latest version {latest}. On the node where Tolato runs, go to the directory containing docker-compose.yaml and run: docker compose pull && docker compose up -d. Then confirm the tolato-server container restarted on the new image.',
    upgradePromptNode:
      'Please upgrade Tolato to the latest version {latest} on node "{node}". Go to the directory containing docker-compose.yaml and run: docker compose pull && docker compose up -d. Then confirm the tolato-server container restarted on the new image.',
    copyPrompt: 'Copy prompt',
    copied: 'Copied',
    viewNotes: 'Release notes',
  },

  nodeDetail: {
    whoCanAccess: 'Who can access',
    accessNone: 'Nobody can reach this node yet.',
    viaAdminRole: 'admin role',
    nodeGroup: 'Node group',
    hostname: 'Hostname',
    ip: 'IP',
    os: 'OS',
    kernel: 'Kernel',
    agentVersion: 'Agent Version',
    cpu: 'CPU',
    memory: 'Memory',
    disk: 'Disk',
    cores: '{count} cores',
    gb: '{value} GB',
    realtimeMetrics: 'Real-time Metrics',
    cpuUsage: 'CPU Usage',
    memoryUsage: 'Memory Usage',
    diskUsage: 'Disk Usage',
    commandHistory: 'Command History',
    command: 'Command',
    exitCode: 'Exit Code',
    noCommandHistory: 'No command history',
    subscriptionInfo: 'Subscription & Notes',
    expiresInDays: 'Expires in {days}d',
    expired: 'Expired {days}d ago',
    updateAgent: 'Update Agent',
    updatingAgent: 'Updating…',
    updateAgentConfirm: 'Update this node\'s agent to the latest version? It will briefly go offline and reconnect.',
    updateAgentSuccess: 'Agent updated: {old} → {new}',
    updateAgentNoChange: 'Agent already up to date ({version})',
    updateAgentFailed: 'Agent update failed: {error}',
  },

  auditLog: {
    actor: 'Actor',
    title: 'Audit Log',
    allNodes: 'All Nodes',
    searchPlaceholder: 'Search commands...',
    node: 'Node',
    command: 'Command',
    confirmed: 'Confirmed',
    stdout: 'stdout',
    stderr: 'stderr',
    durationMs: 'Duration: {ms}ms',
    noLogs: 'No audit logs found',
    failedToLoad: 'Failed to load audit logs',
  },

  settings: {
    title: 'Settings',
    tabs: {
      llm: 'LLM Config',
      security: 'Security',
      agent: 'Node Agent',
      chat: 'Conversation',
      webFetch: 'Web Fetch',
      notify: 'Offline Alerts',
      apiKeys: 'API Keys',
      oidc: 'Single Sign-On',
      account: 'Account',
    },
    oidc: {
      groupSync: 'Group sync',
      groupSyncHelp: 'Keep Tolato user groups in step with your identity provider, so access follows the directory.',
      groupClaim: 'Group claim',
      groupClaimHelp: 'The ID-token claim carrying the user’s groups, commonly "groups". Leave empty to disable group sync.',
      idpGroup: 'Identity provider group',
      tolatoGroup: 'Tolato user group',
      addMapping: 'Add mapping',
      noUserGroups: 'Create a user group on the Permissions page first, then map it here.',
      title: 'Single Sign-On (OIDC)',
      description: 'Let people sign in with your identity provider. Password sign-in keeps working alongside it.',
      enable: 'Enable single sign-on',
      enableHelp: 'Shows an SSO button on the login page.',
      redirectUrl: 'Redirect URL',
      redirectUrlHelp: 'Register this exact URL with your provider as the allowed callback.',
      issuer: 'Issuer URL',
      issuerHelp: 'The base URL. Its /.well-known/openid-configuration is read for endpoint discovery.',
      clientId: 'Client ID',
      clientSecret: 'Client Secret',
      scopes: 'Scopes',
      scopesHelp: 'Space separated. Defaults to openid profile email; openid is always requested.',
      allowSignup: 'Create accounts on first sign-in',
      allowSignupHelp: 'Off means only people who already have a Tolato account can use SSO.',
      adminEmails: 'Administrator emails',
      adminEmailsHelp: 'New accounts with one of these verified addresses become administrators. Everyone else is a member.',
      verifyOk: 'Discovery succeeded.',
      verifyFailed: 'Could not reach the issuer.',
    },
    account: {
      title: 'Account',
      description: 'Your sign-in details.',
      signedInAs: 'Signed in as',
      role: 'Role',
      changePassword: 'Change Password',
      currentPassword: 'Current Password',
      newPassword: 'New Password',
      confirmPassword: 'Confirm New Password',
      passwordMismatch: 'The two passwords do not match.',
      passwordChanged: 'Password changed.',
      failedToChange: 'Failed to change password.',
      ssoNoPassword: 'You sign in through single sign-on, so there is no local password to change.',
      signOut: 'Sign out',
      signOutHint: 'Signs out this browser only. Command-line sessions end with `tolato auth logout`.',
    },
    llm: {
      title: 'LLM Configuration',
      description: 'Configure the AI model provider for your assistant.',
      apiBaseUrl: 'API Base URL',
      apiKey: 'API Key',
      connectionVerified: 'Connection verified',
      connectionFailed: 'Connection failed',
      defaultModel: 'Default Model',
      selectModel: 'Select a model',
      maxRounds: 'Max Rounds',
      temperature: 'Temperature',
      interleavedThinking: 'Interleaved Thinking',
      interleavedThinkingDescription: 'Pass the model\'s reasoning back between tool calls. Enable for DeepSeek thinking models (e.g. deepseek-reasoner); leave off for models that don\'t support it.',
    },
    security: {
      title: 'Security Settings',
      description: 'Configure confirmation requirements and command restrictions.',
      requireConfirmation: 'Require Confirmation',
      confirmDescription: 'Ask for confirmation before executing sensitive commands.',
      sensitiveKeywords: 'Sensitive Keywords',
      addKeyword: 'Add keyword...',
      commandBlacklist: 'Command Blacklist',
      addCommand: 'Add command...',
    },
    agent: {
      title: 'Node Agent Settings',
      description: 'Configure agent behavior on managed nodes.',
      heartbeatInterval: 'Heartbeat Interval (seconds)',
      commandTimeout: 'Command Timeout (seconds)',
      outputMaxLines: 'Output Max Lines',
    },
    conversation: {
      title: 'Conversation Settings',
      description: 'Configure chat behavior and system prompt.',
      contextRounds: 'Context Rounds',
      contextRoundsHelp: 'Number of previous message rounds to include as context.',
      outputTruncateLines: 'Output Truncate Lines',
      customSystemPrompt: 'Custom System Prompt',
      systemPromptPlaceholder: 'Enter a custom system prompt for the AI assistant...',
    },
    webFetch: {
      title: 'Web Fetch',
      description: 'Lets the assistant fetch a public URL when the user provides one. Choose a backend and configure its credentials.',
      mode: 'Backend',
      modeLocalLabel: 'Local (coming soon)',
      modeJinaHint: 'Jina Reader fetches and converts the URL to Markdown on its servers, then returns the result. The URL you provide is sent to Jina.',
      jinaApiKey: 'Jina API Key',
      jinaKeyHint: 'Get a free API key (1M tokens / month, no credit card) at',
      timeoutSec: 'Timeout (seconds)',
      maxKb: 'Max Response (KB)',
      connectionVerified: 'Connection verified',
      connectionFailed: 'Connection failed',
    },
    notify: {
      title: 'Offline Notifications',
      description: 'Send a notification when a node goes offline (or recovers). Supports Telegram, Discord, Slack, WeCom, Feishu, and custom webhooks.',
      thresholdLabel: 'Offline threshold (seconds)',
      thresholdHelp: 'A node with no heartbeat for this long is marked offline (recommended ≥ 2× heartbeat interval). Set 0 to disable background detection.',
      graceLabel: 'Startup grace (seconds)',
      graceHelp: 'Suppress offline alerts for this long after a server restart, so a restart does not fire a storm.',
      recoverLabel: 'Recovery notifications',
      recoverHelp: 'Also notify when a node comes back online.',
      channels: 'Channels',
      channelsHelp: 'Enable multiple channels at once — each event is sent to all enabled channels.',
      addChannel: 'Add channel',
      noChannels: 'No notification channels configured yet.',
      channelName: 'Channel name',
      channelNamePlaceholder: 'e.g. ops-telegram',
      platform: 'Platform',
      enabled: 'Enabled',
      test: 'Send test',
      testSent: 'Test notification sent',
      testFailed: 'Test failed',
      saveBeforeTest: 'Save before testing',
      credentials: 'Credentials',
      tokenStep: 'Token exchange (pre-step)',
      tokenStepHelp: 'This platform first exchanges credentials for an access_token, then sends the message; the token is cached and auto-refreshed.',
      custom: {
        method: 'Method',
        url: 'Request URL',
        headers: 'Headers (JSON)',
        body: 'Body template',
        bodyHelp: "Placeholders: {'{{message}}'} {'{{node_name}}'} {'{{node_ip}}'} {'{{type}}'} {'{{time}}'}, etc.; two-step channels can use {'{{token}}'}.",
      },
      tokenSource: {
        url: 'Token exchange URL',
        body: "Request body (placeholders ok, e.g. {'{{app_id}}'})",
        tokenPath: 'Token field path',
        expiresPath: 'Expiry field path',
        ttl: 'Fallback TTL (s)',
        invalidatePath: 'Invalidate field',
        invalidateValues: 'Invalid values (comma)',
        help: 'On send, if the invalidate field matches any of these values (or HTTP 401), the token is refreshed and the send is retried once.',
      },
      remove: 'Remove',
    },
    apiKeys: {
      owner: 'Owner',
      title: 'API Keys',
      description: 'Manage API keys for external integrations and MCP tools.',
      createKey: 'Create API Key',
      keyCreated: 'API Key Created',
      key: 'Key',
      permission: 'Permission',
      lastUsed: 'Last Used',
      readonly: 'Read-only',
      writable: 'Read & write',
      permissionHelp: 'Read-only: list and inspect nodes. Read & write: also run commands, with confirmation required for sensitive ones. Neither tier can change node attributes.',
      namePlaceholder: 'e.g., MCP Integration',
      copyWarning: "Copy your API key now. It won't be shown again.",
      noKeys: 'No API keys created yet',
    },
    failedToLoad: 'Failed to load settings',
    saveFailed: 'Save failed',
  },

  users: {
    viewAccess: 'View access',
    accessFor: 'Access for {name}',
    accessViaAdminRole: 'Administrators reach every node by role, without needing grants.',
    accessNone: 'This user has no access to any node yet.',
    failedToLoadAccess: 'Failed to load access.',
    title: 'Users',
    description: 'Accounts that can sign in to this instance.',
    addUser: 'Add User',
    username: 'Username',
    displayName: 'Display Name',
    password: 'Password',
    passwordHint: 'At least 8 characters.',
    newPassword: 'New Password',
    role: 'Role',
    roleAdmin: 'Administrator',
    roleMember: 'Member',
    lastLogin: 'Last Sign-in',
    you: 'You',
    disabled: 'Disabled',
    disable: 'Disable',
    enable: 'Enable',
    resetPassword: 'Reset Password',
    resetPasswordFor: 'Reset password for {name}',
    passwordReset: 'Password reset.',
    deleteUser: 'Delete User',
    deleteWarning: 'Delete {name}? Their conversations and API keys are removed with the account. Audit history is kept.',
    created: 'User created.',
    deleted: 'User deleted.',
    failedToLoad: 'Failed to load users.',
    failedToCreate: 'Failed to create user.',
    failedToUpdate: 'Failed to update user.',
    failedToDelete: 'Failed to delete user.',
  },

  permissions: {
    title: 'Permissions',
    description: 'Decide who can reach which machines, and what they may do there.',
    tabGrants: 'Grants',
    tabUserGroups: 'User Groups',
    tabNodeGroups: 'Node Groups',
    grantsHelp: 'Each grant gives a user or user group a level of access to a node, a node group, or every node.',
    userGroupsHelp: 'Group people so you can grant access once instead of per person.',
    nodeGroupsHelp: 'Group machines so a grant covers them all — including machines enrolled later.',
    addGrant: 'Add Grant',
    addGroup: 'Add Group',
    editGroup: 'Edit Group',
    deleteGroup: 'Delete Group',
    deleteGroupWarning: 'Delete {name}? Its grants are removed too. Members themselves are untouched.',
    edit: 'Edit',
    who: 'Who',
    what: 'What',
    level: 'Level',
    members: 'Members',
    groupDescription: 'Description',
    choose: 'Choose…',
    filterPlaceholder: 'Filter…',
    selectAll: 'Select all',
    clearSelection: 'Clear',
    selectedCount: '{count} selected — one grant each.',
    grantsSaved: 'Saved {count} grants.',
    allNodes: 'All nodes',
    kindUser: 'user',
    kindUserGroup: 'user group',
    kindNode: 'node',
    kindNodeGroup: 'node group',
    levelViewer: 'Viewer',
    levelOperator: 'Operator',
    levelManager: 'Manager',
    levelHelp: 'Viewer sees the node and its history. Operator can also run commands and open a terminal. Manager can also rename, upgrade and delete it.',
    noGrants: 'No grants yet. Administrators always reach every node.',
    noGroups: 'No groups yet.',
    noCandidates: 'Nothing to add yet.',
    failedToLoad: 'Failed to load permissions.',
    failedToSave: 'Failed to save.',
    failedToDelete: 'Failed to delete.',
  },
}
