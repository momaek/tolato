  ...
  return {
    ...
    autoExecutionHint:
      detail?.approvalStatus === 'not_required'
        ? 'Low-risk plan will continue without approval.'
        : undefined,
    ...
    switch (kind) {
      ...
      case 'target_confirmation': {
        ...
        return {
          ...
          basis:
            'Backend resolved a target set and is waiting for confirmation.',
          scope:
          ...
          inheritedHint:
            rowTarget.source === 'session_context'
              ? 'Using confirmed target context from this session.'
              : undefined,
        } satisfies TimelineRow
      }
...
export class RealWSClient implements WSClient {
  ...
  disconnect() {
    ...
    this.pending.forEach((request) => {
      request.reject(new Error('ws/ui disconnected'))
    })
    this.pending.clear()
  ...
  private openSocket() {
    return new Promise<void>((resolve, reject) => {
      ...
      socket.onerror = () => {
        settleReject(new Error('ws/ui connection failed'))
      }
      socket.onopen = () => {
        opened = true
        if (!settled) {
      ...
      socket.onclose = () => {
        this.handleSocketClosed(
          opened ? 'ws/ui disconnected' : 'ws/ui closed before ready',
        )
        if (!opened) {
          settleReject(new Error('ws/ui closed before ready'))
        }
      }
    })
  }
